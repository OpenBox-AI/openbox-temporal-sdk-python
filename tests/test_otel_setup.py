...
class TestShouldIgnoreUrl:
    ...
    def test_returns_false_for_non_matching_urls(self):
        ...
        assert (
            _should_ignore_url("http://api.openbox.ai/v1") is False
        )  # Different scheme
...
class TestSetupOpentelemetryForGovernance:
        ...
        with patch.object(otel_setup, "setup_httpx_body_capture"):
            with patch("opentelemetry.trace.get_tracer_provider") as mock_get_provider:
                with patch(
                    ...
                    mock_get_provider.return_value.__class__.__name__ = (
                        "NoOpTracerProvider"
                    )
    ...
    def test_calls_setup_database_instrumentation_when_enabled(
        self, mock_span_processor
    ):
        ...
        with patch.object(otel_setup, "setup_httpx_body_capture"):
            with patch.object(
                otel_setup, "setup_database_instrumentation"
            ) as mock_db_setup:
    ...
    def test_calls_setup_file_io_instrumentation_when_enabled(
        self, mock_span_processor
    ):
...
class TestSetupFileIoInstrumentation:
    ...
    def test_traced_file_wrapper_for_read(self, tmp_path):
        ...
        from openbox.otel_setup import (
...
class TestSetupDatabaseInstrumentation:
    ...
    def test_sqlalchemy_engine_rejects_non_engine_type(self):
        ...
        with patch("opentelemetry.instrumentation.sqlalchemy.SQLAlchemyInstrumentor"):
            with pytest.raises(
                TypeError, match="must be a sqlalchemy.engine.Engine instance"
            ):
    ...
    def test_sqlalchemy_engine_rejects_when_sqlalchemy_not_installed(self):
        ...
        with patch("opentelemetry.instrumentation.sqlalchemy.SQLAlchemyInstrumentor"):
            with patch.dict(
                "sys.modules", {"sqlalchemy": None, "sqlalchemy.engine": None}
            ):
    ...
    def test_warns_when_engine_provided_but_sqlalchemy_not_in_db_libraries(self):
        ...
        with patch(
            ...
            with patch.object(
                logging.getLogger("openbox.otel_setup"), "warning"
            ) as mock_warn:
                setup_database_instrumentation(
                    db_libraries={"psycopg2"},
                    sqlalchemy_engine=MagicMock(),
    ...
    def test_warns_when_engine_provided_but_databases_disabled(
        self, mock_span_processor
    ):
    ...
    def test_sqlalchemy_engine_passthrough_from_setup_governance(
        self, mock_span_processor
    ):
...
class TestUninstrumentAll:
    ...
    def test_handles_import_errors_gracefully(self):
        ...
        mock_requests_module.RequestsInstrumentor.side_effect = ImportError(
            "Mock import error"
        )
...
class TestHttpxAsyncHooks:
    ...
    @pytest.mark.asyncio
    async def test_async_response_hook_ignores_url(
        self, mock_span_processor, mock_span
    ):
...
class TestGovernanceStageField:
    ...
    def test_evaluate_governance_sync_includes_stage_started(
        self, governance_setup, mock_span
    ):
        ...
        with patch(
            "openbox.hook_governance._get_sync_client",
            return_value=mock_client_instance,
        ):
            span_data = _build_http_span_data(
                mock_span, "GET", "https://api.example.com/data", "started"
            )
            evaluate_sync(
                mock_span,
    ...
    def test_evaluate_governance_sync_started_then_completed_sends_2_spans(
        self, governance_setup, mock_span
    ):
        ...
        with patch(
            ...
            first_span = first_payload["spans"][0]
            ...
            assert second_spans[0]["response_headers"] == {
                "content-type": "application/json"
            }
            assert second_spans[0]["http_status_code"] == 200
            second_span = second_payload["spans"][0]
            assert second_span["stage"] == "completed"

    def test_evaluate_governance_sync_omits_status_code_when_none(
        self, governance_setup, mock_span
    ):
    ...
    def test_requests_request_hook_sends_stage_started(
        self, governance_setup, mock_span
    ):
    ...
    def test_requests_response_hook_sends_stage_completed(
        self, governance_setup, mock_span
    ):
    ...
    def test_httpx_response_hook_does_not_call_governance(
        self, governance_setup, mock_span
    ):
    ...
    @pytest.mark.asyncio
    async def test_httpx_async_request_hook_sends_stage_started(
        self, governance_setup, mock_span
    ):
    ...
    @pytest.mark.asyncio
    async def test_httpx_async_response_hook_does_not_call_governance(
        self, governance_setup, mock_span
    ):
    ...
    def test_urllib3_request_hook_sends_stage_started(
        self, governance_setup, mock_span
    ):
    ...
    def test_urllib3_response_hook_sends_stage_completed(
        self, governance_setup, mock_span
    ):
    ...
    def test_response_hook_no_governance_when_disabled(
        self, mock_span_processor, mock_span
    ):
...
class TestContextVarHttpSpanBridging:
    ...
    def test_httpx_request_hook_stores_span_in_contextvar(
        self, mock_span_processor, mock_span
    ):
    ...
    @pytest.mark.asyncio
    async def test_httpx_async_request_hook_stores_span_in_contextvar(
        self, mock_span_processor, mock_span
    ):
