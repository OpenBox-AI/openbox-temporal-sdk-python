...
def setup_opentelemetry_for_governance(
    ...
    *,
    ...
    Args:
        ...
        instrument_http: Whether to install global HTTP instrumentors and the httpx
            body-capture monkeypatch (default: True). False does not uninstall hooks
        ...
        register_span_processor: Register with (or create) a global tracer provider.
                          Defaults to True for exact existing behavior. The governed-
                          command v3 bridge sets this False and disables every
                          instrumentor so setup only configures governance hooks.
    """
    global _span_processor, _ignored_url_prefixes, _instrument_http
    _span_processor = span_processor
    _instrument_http = instrument_http
    ...
    if register_span_processor:
        # Delegate provider ownership to the shared SDK: reuse application
        # providers by capability and never clobber a custom global provider.
        # The local processor remains Temporal-specific correlation state.
        ...
        if callable(add_processor):
            add_processor(cast(Any, span_processor))
            logger.info("Registered WorkflowSpanProcessor with OTel TracerProvider")
        else:
            logger.info(
                "TracerProvider cannot accept the Temporal span processor; "
                "leaving application ownership unchanged"
            )
    ...
    if instrument_http:
        ...
        logger.info(
            "OpenTelemetry HTTP instrumentation disabled; existing process-global "
            "instrumentation, if any, is unchanged"
        )
    ...
    # 8. Context propagation for instrumented async activities. Keep exact
    # default behavior, but install no executor monkeypatch for the fully-local
    # v3 path where registration and all instrumentors are disabled.
    if (
        register_span_processor
        or instrument_http
        or instrument_databases
        or instrument_file_io
    ):
