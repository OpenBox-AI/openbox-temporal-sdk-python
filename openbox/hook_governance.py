...
def set_evaluation_client(client: Any) -> None:
...
def _get_sync_client() -> httpx.Client:
    ...
    if _sync_client is None or _sync_client.is_closed:
        with _sync_client_lock:
            if _sync_client is None or _sync_client.is_closed:
                options = (
                    {"verify": _core_ssl_context}
                    if _core_ssl_context is not None
                    else {}
                )
                _sync_client = httpx.Client(timeout=_api_timeout, **options)
    return _sync_client
    ...
    if _async_client is None or _async_client.is_closed:
        with _async_client_lock:
            if _async_client is None or _async_client.is_closed:
                ...
                _async_client = httpx.AsyncClient(timeout=_api_timeout, **options)
...
def _check_activity_abort(span) -> str | None:
    ...
    if (
        processor is None
        or not hasattr(processor, "get_activity_abort")
        or not callable(getattr(processor, "get_activity_abort", None))
    ):
        return None
    ids = _resolve_activity_ids(span)
    if not ids:
        return None
    result = processor.get_activity_abort(ids[0], ids[1])
...
def _set_activity_abort(span, reason: str) -> None:
    ...
    ids = _resolve_activity_ids(span)
    processor = _span_processor
    if not ids or processor is None:
        return
...
def evaluate_sync(
    ...
    if _evaluation_client is not None:
        ...
        try:
            result = _evaluation_client.evaluate(payload)
        except GovernanceAPIError as error:
            raise GovernanceBlockedError("halt", str(error), identifier) from error
...
async def evaluate_async(
    ...
    if _evaluation_client is not None:
        ...
        try:
            result = await _evaluation_client.aevaluate(payload)
        except GovernanceAPIError as error:
            raise GovernanceBlockedError("halt", str(error), identifier) from error
        if not result.fallback_used:
            data = dict(result.raw)
